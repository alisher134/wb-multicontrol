...
```html
<div class="flex min-w-0 items-center gap-2 font-mono text-sm">
  <span class="i-ph-file-ts-duotone color-blue-500 shrink-0"></span>
  <span
    class="truncate"
    title="/repo/node_modules/.pnpm/react@18.3.1/node_modules/react/index.js"
  >
...
const byName = {
  ...
  '.env': {
    ...
    color: 'color-amber-600 dark:color-amber-400',
  },
}
...
export function fileIcon(path: string) {
  ...
  return (
    byExt[ext as keyof typeof byExt] || {
...
```html
<time
  class="text-micro font-mono tabular-nums op-fade"
  title="2026-06-22 09:14:12"
>
  8m
</time>
```
...
```html
<span class="text-micro op-mute font-mono tabular-nums">12,480</span>
```
...
```html
<span
  class="text-micro inline-flex items-center gap-1 rounded border border-emerald-500/40 bg-emerald-500/10 px-1.5 py-px tracking-wide text-emerald-700 uppercase dark:text-emerald-300"
>
...
// @unocss-include
type SessionStatus =
  'thinking' | 'tool' | 'task' | 'questions' | 'error' | 'idle'
